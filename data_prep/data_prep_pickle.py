...
def load_stations_mapping(file_path: str) -> Dict[str, int]:
    station_ids = pd.read_csv(file_path, encoding='iso-8859-1', dtype=str).station_id.values
    return {val: ind for ind, val in enumerate(station_ids)}


def create_sparse_matrix(data: pd.DataFrame, min_datetime: datetime, max_datetime: datetime,
                         stations_mapping: Dict[str, int]) -> coo_matrix:
...
def load_stations_data(file_path: str) -> pd.DataFrame:
    dtypes = {"datetime_from": str, "start_station_id": str, "end_station_id": str, "hour_from": int}
...
def get_min_max_dateime_array(first_timestamp: pd.Timestamp,
                              last_timestamp: pd.Timestamp) -> List[Tuple[datetime, datetime]]:
    result = []
    min_time = datetime(first_timestamp.year, first_timestamp.month, first_timestamp.day, first_timestamp.hour)
...
def write_pickle_files(stations_data: pd.DataFrame, stations_mapping: Dict[str, int], path: str) -> None:
    Path(path).mkdir(parents=True, exist_ok=True)
    time_array = get_min_max_dateime_array(stations_data.index[0], stations_data.index[-1])

    tmp_hours, tmp_sparse_array = [], []
    for min_datetime, max_datetime in tqdm.tqdm(time_array):
        tmp_sparse_array.append(create_sparse_matrix(stations_data, min_datetime, max_datetime, stations_mapping))
        ...
        if max_datetime.day == 1 and max_datetime.hour == 0:
            ...
            with open(os.path.join(path, file_name), "wb") as file:
                pickle.dump({"feature": tmp_sparse_array, "label": tmp_hours}, file)
            tmp_hours, tmp_sparse_array = [], []
